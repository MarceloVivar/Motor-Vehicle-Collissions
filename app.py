...
injured_people = st.slider("Number of people injured in vehicles", 0,19)
st.map(data.query("injured_persons >= @injured_people")[["latitude", "longitude"]].dropna(how= "any"))

st.header("How many collisinsoccur during a giving time of day?")
...
st.subheader( "Breackdown by minute between %i:00 and %i:00 " % (hour, (hour + 1) % 24))
filtered = data[(data["date/time"].dt.hour>=hour) & (data["date/time"].dt.hour < (hour+1)) ]    
hist =np.histogram(filtered["date/time"].dt.minute, bins=60, range=(0,60))[0]
chart_data = pd.DataFrame({'minute':range(60), 'crashes':hist})
#fig=plt.hist(chart_data, x='minute', X='crashes', hover_data=['minute','crashes'],height=400)
st.write(chart_data)
